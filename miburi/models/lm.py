...
class LMModel(StreamingContainer):
    ...
    def forward_depformer_training(
        ...
        # sequence: B x K x T (except last token in K dim and first token is text token concated in K dim)
        ...
        # logits is [B, Ka, T, card], here loss is applied with the GT audio codes (Ka)
        return logits
...
class LMGen(StreamingModule[_LMGenState]):
    def __init__(
        ...
        # self.offsets = []
        # def print_offset_hook(module, inp, oup):
        #     self.offsets.append(module._streaming_state.offset)
        # lm_model.transformer.register_forward_hook(print_offset_hook)
    ...
    def step(self, input_tokens: torch.Tensor) -> torch.Tensor | None:
        ...
        # GestureLM: you would need some kind of cache here for both texttransformer_out, audio_tokens and maybe text_token
        # this cache will have length of spacing between gesture and audio tokens i.e 5 (4 previous + 1 current)
        # GestureLM: the forward pass will take the last 5 audio token grid and corresponding texttransformer_out [B, T, 1, D] 
        # and previous gesture token to generate next gesture token
        # the forward pass can use embeddings to convert audio/text tokens to normal
        # should we take transformer_out [B, T, K, D] from depformer too? and pass it gesture forward pass?
        # draw this out
        ...
        if state.offset <= self.max_delay:
            # print("Not returned")# print(text_token, audio_tokens)
            return None
