...
class ScaledEmbeddingwithPadEmbedding(nn.Embedding):
    """Boost learning rate for embeddings (with `scale`).
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int,
        ...
        if norm:
            self.norm = create_norm_fn("layer_norm", self.embedding_dim)
        assert zero_idx < 0, "Please use negative values for the zero_idx."
        assert pad_idx > 0 and pad_idx == num_embeddings, "Please use positive values for the pad_idx and set it to num_embeddings."
        self.zero_idx = zero_idx
        self.pad_idx = pad_idx
        self.lr = lr
        self.low_rank = None
        self.pad_embedding = ScaledEmbedding(1, embedding_dim, norm=norm)
        if low_rank is not None:
            self.low_rank = nn.Linear(low_rank, embedding_dim, bias=False)

    def forward(self, input, *args, **kwargs):
        is_zero = input == self.zero_idx
        is_pad = input == self.pad_idx
        zero = torch.zeros(1, dtype=input.dtype, device=input.device)
        # if is_pad.any():
        pad_embedding = self.pad_embedding(torch.zeros_like(input))
        input = input.clamp(min=0)
        input = torch.where(is_pad, torch.zeros_like(input), input) # set pad idx to 0 to get the main embedding temporarily
        y = super().forward(input, *args, **kwargs)
        if self.norm is not None:
            y = self.norm(y)
        ...
        y = torch.where(is_zero[..., None], zero, y)
        # if is_pad.any():
        y = torch.where(is_pad[..., None], pad_embedding, y)
        if self.low_rank is not None:
            y = self.low_rank(y)
        return y
