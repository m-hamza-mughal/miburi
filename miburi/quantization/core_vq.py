...
class FiniteScalarQuantization(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.keep_num_codebooks_dim = False
        ...
        requires_projection = effective_codebook_dim != dim
        self.project_in = (
            nn.Linear(dim, effective_codebook_dim) if requires_projection else nn.Identity()
        )
        self.project_out = (
            nn.Linear(effective_codebook_dim, dim) if requires_projection else nn.Identity()
        )
        self.requires_projection = requires_projection

        self.codebook_size = self._levels.prod().item()
        implicit_codebook = self._codeindices_to_codevec(torch.arange(self.codebook_size))
    ...
    def _codeindices_to_codevec(self, indices):
        level_indices = self.indices_to_level_indices(indices)
        codevec = self._scale_and_shift_inverse(level_indices)
        return codevec
    ...
    def codevecs_to_indices(self, zhat):
        """ Converts a `codevec` to an index in the codebook. """
        assert zhat.shape[-1] == self.codebook_dim
        zhat = self._scale_and_shift(zhat)
        return (zhat * self._basis).sum(dim = -1).round().to(torch.int32)
    ...
    def decode(self, codes: torch.Tensor) -> torch.Tensor:
        ...
        code_indices = codes
        codevecs = self._codeindices_to_codevec(code_indices)

        if self.keep_num_codebooks_dim:
            codevecs = rearrange(codevecs, '... c d -> ... (c d)')

        codevecs = self.project_out(codevecs)

        # if is_img_or_video or self.channel_first:
        
        # codevecs = self._rearrange_output(codevecs)

        return codevecs
    
    def encode(self, z: torch.Tensor) -> torch.Tensor:
        """ Encodes `z` into discrete integer codes. """
        assert exists(z)
        # z = self._rearrange_input(z)
        z = self.project_in(z)
        z = rearrange(z, 'b n (c d) -> b n c d', c = self.num_codebooks)
        z = self.quantize(z)
        indices = self.codevecs_to_indices(z)
        indices = indices.squeeze(-1)  # remove the last dimension
        return indices
    ...
    def forward(self, z: torch.Tensor) -> _VQForwardResult:
        """
        einstein notation
        b - batch
        ...
        # breakpoint () # checkk the shape (batch, seq, dimension)
        # z = self._rearrange_input(z)
        ...
        codevecs = self.quantize(z)

        # returning indices could be optional

        indices = self.codevecs_to_indices(codevecs)

        codevecs = rearrange(codevecs, 'b n c d -> b n (c d)')

        codevecs = codevecs.to(orig_dtype)

        # project out

        quantized = self.project_out(codevecs)
        ...
        metrics = {'fsq_perplexity': perplexity, 'fsq_activate': activate}

        return _VQForwardResult(quantized, indices, zero_scalar(z.device), metrics)


class ResidualFSQ(nn.Module):
    """Residual FSQ implementation.
    ...
    def __init__(self, *, num_quantizers: int, **kwargs):
        ...
        assert levels is not None, 'levels must be provided'
        ...
        for ind in range(num_quantizers):
            ...
            self.layers.append(FiniteScalarQuantization(dim=codebook_dim, levels=levels, **kwargs))

        assert all([not fsq.requires_projection for fsq in self.layers])
    ...
    def forward(
        ...
        for i, (layer, scale) in enumerate(zip(self.layers[:n_q], self.scales[:n_q])):  # type: ignore
            # We only allow the kmeans initialization if the previous layer is already initialized from the previous
            # iterations, this is to avoid learning the subsequent kmeans on the same batch, which would eventually
            # lead to its exhaustion and running kmeans on 0 values.
            quantized, codes, loss, metrics = layer(
        ...
        quantized_out = self._rearrange_output(quantized_out)
    ...
    def encode(self, x: torch.Tensor, n_q: tp.Optional[int] = None) -> torch.Tensor:
        ...
        all_indices = []
        n_q = n_q or len(self.layers)
        for layer, scale in zip(self.layers[:n_q], self.scales[:n_q]):  # type: ignore
            assert isinstance(layer, FiniteScalarQuantization)
            indices = layer.encode(residual/scale)
            quantized = layer.decode(indices) * scale
            residual = residual - quantized
            all_indices.append(indices)
        out_indices = torch.stack(all_indices)
        return out_indices
    ...
    def decode(self, codes: torch.Tensor) -> torch.Tensor:
        ...
        for idx, layer_codes in enumerate(codes):
            ...
            assert isinstance(layer, FiniteScalarQuantization)
            quantized = quantized + (layer.decode(layer_codes) * scale)
