...
from .core_vq import ResidualVectorQuantization, ResidualFSQ
...
class ResidualVectorQuantizer(BaseQuantizer):
    ...
    def decode_to_latent(self, codes: torch.Tensor) -> torch.Tensor:
        """Decode the given codes to the quantized representation."""
        # codes is [B, K, T], with T frames, K nb of codebooks, vq.decode expects [K, B, T].
        codes = codes.transpose(0, 1)
        quantized = self.vq.decode(codes)
...
class ResidualFiniteScalarQuantizer(BaseQuantizer):
    def __init__(
        self,
        dimension: int = 128,
        input_dimension: tp.Optional[int] = None,
        ...
        self.levels = levels
        self.bins = int(torch.prod(torch.tensor(levels)).item())
    ...
    def forward(self, x: torch.Tensor, frame_rate: int):
        ...
        if self.training and self.q_dropout:
            n_q = self.rng_dropout.randint(1, self.n_q)
        bw_per_q = math.log2(self.bins) * frame_rate / 1000
        quantized, codes, commit_loss, metrics = self.residual_fsq(x, n_q=n_q)
        B, _, _ = quantized.shape
        if self.training and self.no_quantization_rate > 0:
            mask = (torch.rand(B, 1, 1, device=x.device) <= self.no_quantization_rate).float()
            quantized = x * mask + (1 - mask) * quantized
...
# class Gesture_SplitResidualVQ(SplitResidualVectorQuantizer):
#     """Residual Vector Quantizer with separate projections for the first quantizer and the rest.
#     BASED ON: https://github.com/lucidrains/vector-quantize-pytorch.git

#     Args:
#         n_q (int): Number of residual vector quantizers used.
#         n_semantic_q (int): Number of residual vector quantizers used for the semantic quantizer.
#         **kwargs: Arguments to the constructor of `ResidualVQ` that are shared between both.
#     """
...
#             num_quantizers=n_q_semantic, **kwargs
#         )
#         if n_q > self.n_q_semantic:
#             self.rvq_rest = ResidualVQ(
#                 num_quantizers=n_q - n_q_semantic,
#                 **kwargs,
#             )
#         else:
#             self.rvq_rest = None
...
#     @property
#     def total_codebooks(self):
#         return self.rvq_first.num_quantizers + self.rvq_rest.num_quantizers
...
#     @property
#     def semantic_quantizer(self) -> ResidualVQ:
#         """This returns the quantizer that models the first level of the hierarchy (typically semantic)."""
#         return self.rvq_first

#     @property
#     def motion_quantizer(self) -> ResidualVQ:
#         """This returns the quantizer that models the higher levels of the hierarchy (typically acoustic)."""
#         return self.rvq_rest
    
#     @property
#     def acoustic_quantizer(self):
#         raise NotImplementedError("This quantizer does not have an acoustic quantizer.")
    
#     @property
#     def set_num_codebooks(self, n):
#         raise NotImplementedError("This quantizer does not support setting num codebooks.")
    
#     def forward(self, x: torch.Tensor, frame_rate: int):

#         semantic_x, semantic_codes, semantic_commitloss= self.rvq_first(x)

#         # calculate bandwith (kbps)
#         semantic_bw_per_q = math.log2(self.rvq_first.codebook_size) * frame_rate / 1000
#         sem_bw = torch.tensor(self.n_q_semantic * semantic_bw_per_q).to(x)
...
#                 semantic_codes, 
#                 sem_bw, 
#                 penalty=torch.mean(semantic_commitloss), 
#                 metrics={},
#                 sem_emb=semantic_x
#             )
        
#         motion_x, motion_codes, motion_commitloss = self.rvq_rest(x)
#         full_quantized_emb = semantic_x + motion_x
#         full_quantized_codes = torch.cat(
#             [semantic_codes, motion_codes], dim=2
#         )
#         full_quantized_codes = full_quantized_codes.permute(0, 2, 1) # to match mimi's convention

#         full_quantized_penalty = torch.mean(semantic_commitloss) + \
#             torch.mean(motion_commitloss)

        
#         motion_bw_per_q = math.log2(self.rvq_rest.codebook_size) * frame_rate / 1000
#         mot_bw = torch.tensor(self.n_q_acoustic * motion_bw_per_q).to(x)
#         full_bw = sem_bw + mot_bw
...
#         if self.max_n_q > self.n_q_semantic:
#             motion_x, motion_codes, motion_commitloss = self.rvq_rest(x)
#             codes = torch.cat([codes, motion_codes], dim=2)

#         codes = codes.permute(0, 2, 1) # to match mimi's convention
#         # codes is [B, K, T], with T frames, K nb of codebooks.
#         return codes
...
#         if codes.shape[-1] > self.n_q_semantic:
#             motion_result = self.rvq_rest.get_output_from_indices(codes[:, :, self.n_q_semantic :])
#             quantized = quantized + motion_result
#         return quantized