...
try:
    from miburi.conditioners import ConditionAttributes
except Exception:  # pragma: no cover - optional dependency
    class ConditionAttributes:  # type: ignore
        pass
...
class InterleavedTokenizer:
    def __init__(self, mimi, interleaver):
        self.mimi = mimi
    ...
    def __call__(
        ...
        with torch.no_grad():
            audio_tensor = torch.Tensor(wav).to(self.interleaver.device)
            num_audio_frames = math.ceil(duration_sec * self.mimi.frame_rate)
            ...
            audio_tokens = torch.nn.functional.pad(
                ...
                (0, num_audio_frames - this_num_audio_frames),
            ...
            end_alignment = dicho(alignments, start_sec + duration_sec)
            alignments = [
