...
# Speakers whose lower-body motion is unreliable and is masked out by the
# `lower_valid` filter in the BEATX dataloader (see embody3d_dyadic/dataset.py).
# When `--filter_lowervalid_speakers` is on, the build skips the same samples
# so the lowervalid-filtered HDF5 matches what the dataloader would actually use.
LOWERVALID_DROP_SPEAKERS_ALL_SPLITS = frozenset({"itoi", "carla"})
LOWERVALID_DROP_SPEAKERS_ADDITIONAL_ONLY = frozenset(
    {"hailing", "scott", "sophie", "lawrence", "daiki"}
)


def _beatx_speaker_name(file_id: str) -> str:
...
def _build_file_list(args) -> list[FileEntry]:
    ...
    if not args.file_list_path:
        raise NotImplementedError("Directory scanning is not implemented")

    progress_every = max(1, int(getattr(args, "filelist_progress_every", 10000)))
    scanned = 0
    kept = 0
    dropped_lowervalid = 0
    t0 = time.time()
    logger.info("Reading file list from {}", args.file_list_path)
    with open(args.file_list_path, "r") as f:
        ...
        for row in reader:
            ...
            if scanned % progress_every == 0:
                logger.info(
                    "File list progress: scanned={} kept={} dropped_lowervalid={} elapsed={:.1f}s",
                    scanned,
                    kept,
                    dropped_lowervalid,
                    time.time() - t0,
            ...
            archive_idx = "-" # int(row.get("archive_idx"))

            if args.filter_lowervalid_speakers:
                spk = _beatx_speaker_name(file_id)
                if spk in LOWERVALID_DROP_SPEAKERS_ALL_SPLITS:
                    dropped_lowervalid += 1
                    continue
                if split == "additional" and spk in LOWERVALID_DROP_SPEAKERS_ADDITIONAL_ONLY:
                    dropped_lowervalid += 1
                    continue

            if split == "additional": split = "train"
            if split == "val": split = "dev"

            relpath = f"{file_id}"
            rel_motion = relpath + args.motion_ext
            rel_audio = relpath + args.audio_ext
            rel_text = relpath + args.text_ext

            motion_path = os.path.join(args.data_dir, "smplxflame_25", rel_motion)
            audio_path = os.path.join(args.data_dir, "wave16k", rel_audio)
            text_path = os.path.join(args.data_dir, "whisper_transcription", rel_text)
            pose_valid_path = None # motion_path.replace("SMPLX_tracking_outputs", "valid_frames")
            ...
            entries.append(
                FileEntry(
                    ...
                    text_path=text_path,
                    pose_valid_path=pose_valid_path #if os.path.exists(pose_valid_path) else None,
    ...
    logger.info(
        "File list read complete: scanned={} kept={} dropped_lowervalid={} max_files={} elapsed={:.1f}s",
        scanned,
        len(entries),
        dropped_lowervalid,
        str(args.max_files),
...
def _process_entry(entry: FileEntry, args, models: dict[str, Any]) -> dict[str, Any]:
    ...
    if entry.pose_valid_path is not None:
        tracking_valid_bits = np.load(entry.pose_valid_path) # 0 and 1 values indicating valid frames based on tracking confidence
        tracking_valid_bits = tracking_valid_bits['valid_frames']
        if tracking_valid_bits.shape[0] == nframes:
            pose_valid = pose_valid & tracking_valid_bits.astype(bool)
...
def parse_args():
    ...
    parser.add_argument("--data_dir", default="datasets/beat_english_v2.0.0/")
    # parser.add_argument("--motion_dir", default="/CT/GestureSynth2/work/SeamlessInteraction/seamless_retracking")
    parser.add_argument("--file_list_path", default="datasets/beat_english_v2.0.0/train_test_split.csv")
    parser.add_argument("--hdf5_path", required=True)

    parser.add_argument("--motion_ext", default=".npz")
    parser.add_argument("--audio_ext", default=".wav")
    parser.add_argument("--text_ext", default=".json")
